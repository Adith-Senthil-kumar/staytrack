import { formatINR } from '../domain/format';
import { toDateStr } from '../domain/dates';
import type { Tenant, Due } from '../../types';

// A printable rent-payment receipt — pure string builder, safe on any platform.
export function rentReceiptHtml(
  tenant: Tenant,
  roomNumber: string,
  due: Due,
  monthLabel: string,
  propertyName: string,
): string {
  const paidDate = toDateStr(due.paidAt);
  const method = due.method ? due.method.toUpperCase() : null;
  const row = (label: string, value: string) =>
    `<tr><td style="padding:9px 0;color:#71776A;font-size:13px;border-bottom:1px solid #ECE6DA">${label}</td>
      <td style="padding:9px 0;text-align:right;font-family:'SF Mono',ui-monospace,Menlo,monospace;font-size:13px;color:#41463C;border-bottom:1px solid #ECE6DA">${value}</td></tr>`;

  return `<!DOCTYPE html>
<html><head><meta charset="utf-8" />
<title>Rent Receipt · ${tenant.name} · ${monthLabel}</title>
<style>
  @media print { @page { margin: 16mm; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } }
  * { box-sizing: border-box; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #23291F; margin: 0; background: #fff; }
  .sheet { max-width: 460px; margin: 0 auto; }
</style></head>
<body>
  <div class="sheet">
    <div style="background:#13352C;color:#FBF8F0;padding:24px 26px;border-radius:14px 14px 0 0">
      <div style="font-size:11px;letter-spacing:1.4px;text-transform:uppercase;color:#6F9588">Rent Receipt · ${propertyName}</div>
      <div style="font-size:22px;margin-top:4px;font-weight:600">${tenant.name}</div>
      <div style="font-size:12.5px;color:#8FB0A5">Room ${roomNumber} · ${monthLabel}</div>
    </div>
    <div style="border:1px solid #E7E1D4;border-top:none;border-radius:0 0 14px 14px;padding:22px 26px">
      <table style="width:100%;border-collapse:collapse">
        ${row('Month', monthLabel)}
        ${row('Room', roomNumber)}
        ${row('Monthly rent', formatINR(tenant.rent))}
        ${row('Paid on', paidDate)}
        ${method ? row('Method', method) : ''}
      </table>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:14px">
        <div style="font-size:14px;font-weight:700;color:#13352C">Amount Paid</div>
        <div style="font-size:24px;font-weight:700;font-family:'SF Mono',ui-monospace,Menlo,monospace;color:#13352C">${formatINR(due.amountPaid)}</div>
      </div>
      <div style="margin-top:10px;font-size:12px;color:#1E6F5C">✓ Rent received in full${method ? ` via ${method}` : ''}</div>
      <div style="margin-top:24px;padding-top:14px;border-top:1px dashed #DED8C8;font-size:11px;color:#9A9A8A;text-align:center">
        Thank you — ${propertyName}.
      </div>
    </div>
  </div>
</body></html>`;
}
