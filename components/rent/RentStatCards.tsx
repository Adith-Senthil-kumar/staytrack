...
export function RentStatCards({ collected, outstanding, billed, overdueCount, monthLabel }: {
  collected: number; outstanding: number; billed: number; overdueCount: number; monthLabel: string;
}) {
  ...
  return (
    <View className="mb-5 flex-row flex-wrap gap-3 sm:gap-4">
      <View className={card}>
        <Text className="text-xs text-muted">Collected · {monthLabel}</Text>
