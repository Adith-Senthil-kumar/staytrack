...
// Money is stored as integer paise; seed values below are written in rupees and
// scaled here so the demo renders identical amounts to a real (paise) backend.
function scaleMoney<T extends object>(rows: T[], fields: (keyof T)[]): T[] {
  return rows.map((r) => {
    const c = { ...r } as unknown as Record<string, unknown>;
    for (const f of fields as string[]) if (typeof c[f] === 'number') c[f] = (c[f] as number) * 100;
...
const RAW_DEMO_TENANTS: Tenant[] = [
  { id: 't1', name: 'Arjun Mehta', phone: '+91 90000 10001', roomId: 'r301', rent: 6000, deposit: 12000, joinDate: '2025-09-01', status: 'active', foodPreference: 'veg', documents: ['Aadhaar Card', 'PAN Card', 'Police Verification'], documentPhotos: { 'Aadhaar Card': 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAOCAIAAACgpqunAAAAF0lEQVR4nGOwiqogGzGMah7VPKoZEwEAyH4lMGGtIagAAAAASUVORK5CYII=', 'PAN Card': 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAOCAIAAACgpqunAAAAF0lEQVR4nGOwiqogGzGMah7VPKoZEwEAyH4lMGGtIagAAAAASUVORK5CYII=' } },
  ...
  { id: 't8', name: 'Naveen Kumar', phone: '+91 90000 10008', roomId: null, rent: 5000, deposit: 10000, joinDate: '2026-03-12', status: 'vacated', foodPreference: 'veg' },
];

const RAW_DEMO_DUES: Due[] = [
...
const RAW_DEMO_EXPENSES: Expense[] = [
...
const RAW_DEMO_TICKETS: MaintTicket[] = [
...
const RAW_DEMO_SSROOMS: SSRoom[] = [
  { id: 'ss1', number: 'A1', dailyRate: 1200, status: 'available', guestName: null, checkIn: null, checkOut: null },
  { id: 'ss2', number: 'A2', dailyRate: 1200, status: 'occupied', guestName: 'Vikram Singh', checkIn: '2026-06-19', checkInTime: '14:30', checkOut: '2026-06-23', phone: '+91 98801 22334', rate: 1200, advance: 2000, payMethod: 'upi', idType: 'aadhaar', idPhotoUrl: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAOCAIAAACgpqunAAAAF0lEQVR4nGOwiqogGzGMah7VPKoZEwEAyH4lMGGtIagAAAAASUVORK5CYII=' },
...
// Money fields scaled to paise so the demo matches the real (paise) storage format.
export const DEMO_ROOMS = scaleMoney(RAW_DEMO_ROOMS, ['baseRent']);
export const DEMO_TENANTS = scaleMoney(RAW_DEMO_TENANTS, ['rent', 'deposit']);
export const DEMO_DUES = scaleMoney(RAW_DEMO_DUES, ['amountDue', 'amountPaid']);
export const DEMO_EXPENSES = scaleMoney(RAW_DEMO_EXPENSES, ['amount']);
export const DEMO_STAFF = scaleMoney(RAW_DEMO_STAFF, ['salary', 'advance']);
export const DEMO_TICKETS = scaleMoney(RAW_DEMO_TICKETS, ['cost']);
export const DEMO_SSROOMS = scaleMoney(RAW_DEMO_SSROOMS, ['dailyRate', 'rate', 'advance']);
export const DEMO_SSSTAYS = scaleMoney(RAW_DEMO_SSSTAYS, ['total', 'rate', 'advance', 'balance']);
