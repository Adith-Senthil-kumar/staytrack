...
export function SidePanel({ collected, potential, outstanding, net, status, mess }: {
  ...
  const rows: { key: keyof typeof STATUS_UI; count: number }[] = [
    { key: 'occupied', count: status.occupied }, { key: 'pending', count: status.pending },
    { key: 'vacant', count: status.vacant }, { key: 'repair', count: status.repair },
  ];
  return (
    <View className="gap-4">
      ...
      <View className="rounded-[14px] border border-border bg-surface px-5 py-[18px]">
        ...
        {rows.map(({ key, count }) => (
          <View key={key} className="mb-3 flex-row items-center gap-2.5">
            ...
            <Text className="flex-1 text-[13px] text-text-2">{STATUS_UI[key].label}</Text>
            <Text className="font-mono-semibold text-sm text-ink">{count}</Text>
          </View>
        ))}
        <View className="mt-1 h-2.5 flex-row overflow-hidden rounded-md border border-border-2">
      ...
      <View className="rounded-[14px] border border-border bg-surface px-5 py-[18px]">
        ...
        <View className="mb-3 flex-row gap-3">
          <View className="flex-1 rounded-[10px] bg-occ-bg p-3"><Text className="text-[11.5px] font-sans-medium text-label">● Veg</Text><Text className="mt-1 font-mono-semibold text-[22px] text-veg">{mess.veg}</Text></View>
          <View className="flex-1 rounded-[10px] bg-bad-bg p-3"><Text className="text-[11.5px] font-sans-medium text-label">● Non-veg</Text><Text className="mt-1 font-mono-semibold text-[22px] text-nonveg">{mess.nonveg}</Text></View>
        </View>
